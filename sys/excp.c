...
void handle_smode_exception(unsigned int cause, struct trap_frame* tfr) {
    ...
    if (name != NULL) {
        switch (cause) {
            case RISCV_SCAUSE_ECALL_FROM_SMODE: //should never happen
    ...
    if (name != NULL) {
        switch (cause) {
            case RISCV_SCAUSE_ECALL_FROM_UMODE:
                handle_syscall(tfr); //the syscall is already loaded into a7
                return;
            case RISCV_SCAUSE_LOAD_PAGE_FAULT:
                break;
            case RISCV_SCAUSE_STORE_PAGE_FAULT:
                if(handle_umode_page_fault(tfr, (unsigned long)csrr_stval()) == 1){
                    return;
                }
                break;
            case RISCV_SCAUSE_INSTR_PAGE_FAULT:
