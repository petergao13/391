...
smode_trap_entry_from_umode:
        # we have a pointer to a trap frame at the base of the kernerl stack now
        # slide 33 and 32 i think

        # Set up kernel stack pointer (from sscatch)
        
        # Set up tp to point to current thread
        # tp is a general purpose register (may have been modified by user)

        # Keep a pointer to the current thread at the base of the kernel stack:
        # The sscratch register starts pointing to the anchor
        ...
        # store user TP to trap frame
        addi    sp, sp, -TFRSZ
        sd      tp, TP(sp)
        addi    sp, sp, TFRSZ

        # store kernel TP from KTP
        ld      tp, KTP(sp)
        ...
        # load user SP from sscratch
        # store 0 to sscratch
        # store user SP to trap frame
        ...
        # load user SP from trap frame
        # store user SP to sscratch
        ld      t6, SP(sp)
        csrw    sscratch, t6
        ...
        # store kernel TP to KTP
        sd      tp, KTP(sp)

        # load user TP from trap frame
        addi    sp, sp, -TFRSZ
        ld      tp, TP(sp)
        addi    sp, sp, TFRSZ

        # store kernel SP to sscratch
        # load user SP to SP
        csrrw   sp, sscratch, sp
...
smode_trap_entry_from_smode_cont:      
        csrr    a0, scause      # a0 contains "exception code"
        mv      a1, sp          # a1 contains trap frame pointer
