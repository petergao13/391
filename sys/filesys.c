 ...
 */
int parse_path(char* path, char** mpnameptr, char** flnameptr) { //this function is good, checked!
    // use strchr and strncpy instead
    if (path == NULL) {
        return -EINVAL;
    }

    // remove later
    if(*path == '/') path++;        // adjust initial slash if exists

    char * slash = strchr(path, '/');
    if (slash == NULL) {
        return -EINVAL; //no mountpoint found
    }

    char * mount_start = path;
    size_t mount_len = (size_t)slash - (size_t)path;
    
    char * file_start = slash+1;
    size_t file_len = strlen(file_start);

    char * mpname = kcalloc(1, mount_len + 1);
    char * flname = kcalloc(1, file_len + 1);

    strncpy(mpname, mount_start, mount_len);
    strncpy(flname, file_start, file_len);
