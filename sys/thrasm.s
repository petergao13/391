...
#Assembly helper function for thread spawn
#Helper function for thread spawn. It should assist in thread spawn’s goal of ensuring that a newly created
#thread starts in the entry function with the proper arguments, and calls thread exit when entry finishes.
#Implementation: Implementation details are left as a decision for the reader to make. The only “hard requirement” 
#is that your implementation should match with how thread context is expected to be in thread spawn.
...
_thread_startup:
        ...
        #we set pc to entry, so it is held in s8
        #make sure we call thread exit, which is the ra in s11
        mv ra, s11
        jr s8 #go to trap entry
